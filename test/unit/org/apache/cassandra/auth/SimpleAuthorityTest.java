...
public class SimpleAuthorityTest {
    ...
    private final List<Object> KEYSPACES_RESOURCE = Arrays.<Object> asList(
        Resources.ROOT, Resources.KEYSPACES);
    private final List<Object> KEYSPACE1_RESOURCE = Arrays.<Object> asList(
        Resources.ROOT, Resources.KEYSPACES, "Keyspace1");
    private final List<Object> KEYSPACE2_RESOURCE = Arrays.<Object> asList(
        Resources.ROOT, Resources.KEYSPACES, "Keyspace2");
    ...
    public void testAuthorizeKeyspace() throws Exception {
        ...
        assertEquals(
            "a keyspace not listed in the access file should be inaccessible",
            Permission.NONE, authority.authorize(USER1, KEYSPACE2_RESOURCE));
    }

    @Test
    public void testAuthorizeKeyspaceList() throws Exception {
        assertEquals("user1 should be able to modify the keyspace list",
            Permission.ALL, authority.authorize(USER1, KEYSPACES_RESOURCE));
        assertEquals("user2 should only be able to read the keyspace list",
            EnumSet.of(Permission.READ), authority.authorize(USER2,
                KEYSPACES_RESOURCE));
        assertEquals("user3 should only be able to read the keyspace list",
            EnumSet.of(Permission.READ), authority.authorize(USER3,
                KEYSPACES_RESOURCE));
    }
