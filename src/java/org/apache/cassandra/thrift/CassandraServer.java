...
import org.apache.cassandra.auth.KVACUtil;
...
public class CassandraServer implements Cassandra.Iface {
    ...
    private List<ColumnOrSuperColumn> thriftifySuperColumns(
        Collection<IColumn> columns, boolean reverseOrder, boolean isCounterCF) {
        if (isCounterCF)
    ...
    public List<ColumnOrSuperColumn> get_slice_kvac(ByteBuffer key,
        ...
        String columnName = getStringRepresentation(key);

        // state().hasColumnFamilyAndColumnAccess(column_parent.column_family,
        // columnName, Permission.READ);
        // state().hasColumnFamilyAccess(column_parent.column_family,
        // Permission.READ);

        List<ColumnOrSuperColumn> colList = multigetSliceInternal(
            state().getKeyspace(), Collections.singletonList(key),
            column_parent, predicate, consistency_level).get(key);

        // 1. Find the value of the column with columnName read from the policy
        // file. For now hard coding the columnName.
        String queryColName = "name";
        String queryColVal = "";
        for (ColumnOrSuperColumn l : colList) {
            Column col = l.getColumn();
            String colName = this.getStringRepresentation(col.bufferForName());
            if (colName.equals(queryColName)) {
                queryColVal = this
                    .getStringRepresentation(col.bufferForValue());
                logger.info("-- KVAC -- " + queryColVal);
                break;
            }
        }

        // 2. Query other column family, read from the policy file, to query
        // columns from that column family.
        ...
        String temp = this.getStringRepresentation(keyOfAccessor);
        ...
        try {
            ColumnOrSuperColumn accessorsSpecificColumn = internal_get(
                keyOfAccessor, accessorColPath, consistency_level);
    ...
    private ColumnOrSuperColumn internal_get_kvac_prev(ByteBuffer key,
        ...
        UnavailableException, TimedOutException {
        String keyName = getStringRepresentation(key);
        String columnName = getStringRepresentation(column_path.column);

        // state().hasColumnFamilyAndColumnAccess(column_path.column_family,
        // columnName, Permission.READ);
        ...
        ColumnOrSuperColumn colOrSuperCol = tcolumns.get(0);
        Column gotCol = colOrSuperCol.getColumn();
        String colName = getStringRepresentation(gotCol.bufferForName());
        String colValue = getStringRepresentation(gotCol.bufferForValue());

        Map<String, String> resourceNameValueMap = new LinkedHashMap<String, String>();
        resourceNameValueMap.put("keyspace", keyspace);
    ...
    public boolean access(String resourcePath) throws InvalidRequestException,
        NotFoundException, UnavailableException, TimedOutException {
        logger.debug("access");
    ...
    private void internal_insert(ByteBuffer key, ColumnParent column_parent,
        ...
        CharsetDecoder decoder = charset.newDecoder();
        String columnName = null;
        ByteBuffer columnNameByteBuffer = column.bufferForName().duplicate();
        CharBuffer columnNameCharBuffer;
        try {
            columnNameCharBuffer = decoder.decode(columnNameByteBuffer);
            columnName = columnNameCharBuffer.toString();
        } catch (CharacterCodingException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
        }
        ...
        try {
            ...
            try {
                ...
                rows = StorageProxy.getRangeSlice(new RangeSliceCommand(
                    keyspace, column_parent, predicate, bounds, range.count),
                    consistency_level);
            } finally {
                release();
            }
    ...
    public List<TokenRange> describe_ring(String keyspace)
        ...
        if (keyspace == null
            ...
            throw new InvalidRequestException(
                "There is no ring for the keyspace: " + keyspace);
        List<TokenRange> ranges = new ArrayList<TokenRange>();
        Token.TokenFactory tf = StorageService.getPartitioner()
            .getTokenFactory();

        Map<Range, List<InetAddress>> address_map = StorageService.instance
        ...
        for (Map.Entry<Range, List<InetAddress>> entry : address_map.entrySet()) {
            ...
            for (InetAddress endpoint : entry.getValue()) {
                ...
                rpc_endpoints.add(StorageService.instance
                    .getRpcaddress(endpoint));
            }

            TokenRange tr = new TokenRange(tf.toString(range.left), tf
                .toString(range.right), endpoints);
    ...
    public List<String> describe_splits(String cfName, String start_token,
        ...
        List<Token> tokens = StorageService.instance.getSplits(state()
            .getKeyspace(), cfName, new Range(tf.fromString(start_token), tf
            .fromString(end_token)), keys_per_split);
    ...
    public synchronized String system_update_column_family(CfDef cf_def)
        ...
        CFMetaData oldCfm = DatabaseDescriptor.getCFMetaData(CFMetaData.getId(
            cf_def.keyspace, cf_def.name));
        if (oldCfm == null)
            throw new InvalidRequestException(
        ...
        try {
            ...
            UpdateColumnFamily update = new UpdateColumnFamily(CFMetaData
                .convertToAvro(cf_def));
                ...
                case GZIP:
                    ...
                    while (true) {
                        if (decompressor.needsInput())
                            lenRead = query.remaining() < 1024 ? query
                                .remaining() : 1024;
