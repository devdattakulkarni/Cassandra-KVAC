...
import org.apache.cassandra.auth.KVACAuthority;
...
public class DatabaseDescriptor {
    ...
    // private static IAuthenticator authenticator = new
    ...
    //private static IAuthority authority = new KVACAuthority(); //-- Uncomment
    // this when using KVAC
    ...
    static {
        try {
            ...
            if (conf.disk_access_mode == Config.DiskAccessMode.auto) {
                conf.disk_access_mode = System.getProperty("os.arch").contains(
                    "64") ? Config.DiskAccessMode.mmap
            ...
            if (conf.authenticator != null)
                authenticator = FBUtilities.<IAuthenticator> construct(
            ...
            if (conf.concurrent_replicates != null
                ...
                throw new ConfigurationException(
                    "conf.concurrent_replicates must be at least 2");
            }
            ...
            if (conf.listen_address != null) {
                if (conf.listen_address.equals("0.0.0.0")) {
                    throw new ConfigurationException(
                        "listen_address must be a single interface.  See http://wiki.apache.org/cassandra/FAQ#cant_listen_on_ip_any");
                }
            ...
            if (conf.rpc_address != null) {
                try {
                    ...
                    throw new ConfigurationException(
                        "Unknown host in rpc_address " + conf.rpc_address);
                }
            } else {
            ...
            if (conf.thrift_framed_transport_size_in_mb > 0
                ...
                throw new ConfigurationException(
                    "thrift_max_message_length_in_mb must be greater than thrift_framed_transport_size_in_mb when using TFramedTransport");
            }
            ...
            if (conf.compaction_thread_priority < Thread.MIN_PRIORITY
                ...
                throw new ConfigurationException(
                    "compaction_thread_priority must be between 1 and 5");
            }
            ...
            if (logger.isDebugEnabled() && conf.auto_bootstrap != null) {
                logger
                    .debug("setting auto_bootstrap to " + conf.auto_bootstrap);
            }
            ...
            if (conf.concurrent_compactors == null)
                conf.concurrent_compactors = Runtime.getRuntime()
                    .availableProcessors();
            ...
            if (!CassandraDaemon.rpc_server_types.contains(conf.rpc_server_type
                .toLowerCase()))
            ...
            if (conf.commitlog_directory != null
                ...
                if (conf.commitlog_directory
                    .equals(conf.saved_caches_directory))
                    throw new ConfigurationException(
            ...
            KSMetaData systemMeta = new KSMetaData(Table.SYSTEM_TABLE,
                LocalStrategy.class, KSMetaData.optsWithRF(1),
                CFMetaData.StatusCf, CFMetaData.HintsCf,
                CFMetaData.MigrationsCf, CFMetaData.SchemaCf,
                CFMetaData.IndexCf, CFMetaData.NodeIdCf);
    ...
    private static IEndpointSnitch createEndpointSnitch(
        ...
        IEndpointSnitch snitch = FBUtilities.construct(endpointSnitchClassName,
            "snitch");
    ...
    public static void loadSchemas() throws IOException {
        ...
        // real data in the definitions table. If we do end up loading from xml,
        // store the defintions so that we
        ...
        if (uuid == null) {
            ...
            if (hasExistingTables)
                ...
                logger
                    .info("To create keyspaces and column families, see 'help create keyspace' in the CLI, or set up a schema using the thrift system_* calls.");
        } else {
                ...
                for (CFMetaData cfm : def.cfMetaData().values()) {
                    if (!cfm.cfName.matches(Migration.NAME_VALIDATOR_REGEX))
                        throw new RuntimeException(
                            "invalid column family name: " + cfm.cfName);
                    try {
                        CFMetaData.map(cfm);
            ...
            if (tableDefs.size() == 0) {
                logger
                    .warn("No schema definitions were found in local storage.");
    ...
    public static String getInitialToken() {
        return System
            .getProperty("cassandra.initial_token", conf.initial_token);
    }
     ...
     * If the table name or column family name is not valid this function
     * returns null.
     */
    ...
    public static String[] getAllDataFileLocationsForTable(String table) {
        ...
        for (int i = 0; i < conf.data_file_directories.length; i++) {
            tableLocations[i] = conf.data_file_directories[i] + File.separator
                + table;
        }
    ...
    public static int getCommitLogSegmentSize() {
        return conf.commitlog_rotation_threshold_in_mb != null ? conf.commitlog_rotation_threshold_in_mb * 1024 * 1024
            : 128 * 1024 * 1024;
    }
    ...
    public static Set<InetAddress> getSeeds() {
        return Collections
            .unmodifiableSet(new HashSet(seedProvider.getSeeds()));
    }

    /*
     * Loop through all the disks to see which disk has the max free space
     * return the disk with max free space for compactions. If the size of the
     * expected compacted file is greater than the max disk space available
     * return null, we cannot do compaction in this case.
     */
    public static String getDataFileLocationForTable(String table,
        ...
        logger
            .debug(
                "expected data files size is {}; largest free partition has {} bytes free",
    ...
    public static AbstractType getComparator(String ksName, String cfName) {
        ...
        if (cfmd == null)
            throw new IllegalArgumentException("Unknown ColumnFamily " + cfName
                + " in keyspace " + ksName);
    ...
    // todo: this is wrong. the definitions need to be moved into their own
    // class that can only be updated by the
    // process of mutating an individual keyspace, rather than setting manually
    // here.
    ...
    public static void validateMemtableThroughput(int sizeInMB)
        ...
        if (sizeInMB <= 0)
            throw new ConfigurationException(
                "memtable_throughput_in_mb must be greater than 0.");
    }
    ...
    public static void validateMemtableOperations(double operationsInMillions)
        ...
        if (operationsInMillions <= 0)
            throw new ConfigurationException(
                "memtable_operations_in_millions must be greater than 0.0.");
        if (operationsInMillions > Long.MAX_VALUE / 1024 * 1024)
            throw new ConfigurationException(
                "memtable_operations_in_millions must be less than "
                    + Long.MAX_VALUE / 1024 * 1024);
    }
    ...
    public static void validateMemtableFlushPeriod(int minutes)
        ...
        if (minutes <= 0)
            throw new ConfigurationException(
                "memtable_flush_after_mins must be greater than 0.");
    }
